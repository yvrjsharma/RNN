...
X_train_emb_lrn_rnn, X_test_emb_lrn_rnn, Y_train_emb_lrn_rnn, Y_test_emb_lrn_rnn = train_test_split(data, targets,
                                                                                  stratify=targets, 
                                                                                  test_size=0.20)
print("Shape of newly created train and test segments respectively:",X_train_emb_lrn_rnn.shape, X_test_emb_lrn_rnn.shape)
...
vlscores_emb_lrn_rnn=[]
for i in range(shuffle):
    print(i)
    #Further splitting such that final split represents 50% training (25000 instances), 30% validation (15000 instances)
    X_training_emb_lrn_rnn, X_validation_emb_lrn_rnn, Y_training_emb_lrn_rnn, Y_validation_emb_lrn_rnn = train_test_split(X_train_emb_lrn_rnn, Y_train_emb_lrn_rnn,
                                                                                                                              stratify=Y_train, shuffle=True,
                                                                                                                              test_size=0.375)
    print("Size of Training dataset (50%):", len(X_training_emb_lrn_rnn))
    print("Size of Validation dataset (30%):", len(X_validation_emb_lrn_rnn))
    
    #Padding the token sequences to make all of them size
    max_words = 500
    X_training_emb_lrn_rnn = sequence.pad_sequences(X_training_emb_lrn_rnn, maxlen=max_words)
    X_validation_emb_lrn_rnn = sequence.pad_sequences(X_validation_emb_lrn_rnn, maxlen=max_words)
    ...
    hist = model.fit(X_training_emb_lrn_rnn, Y_training_emb_lrn_rnn, validation_data=(X_validation_emb_lrn_rnn, Y_validation_emb_lrn_rnn), 
              batch_size=batch_size, epochs=num_epochs,verbose=1,callbacks=[es])

    #Calculating Accuracy metrics over Validation set 
    scores_emb_lrn_rnn = model.evaluate(X_validation_emb_lrn_rnn, Y_validation_emb_lrn_rnn, verbose=2)
    print("Validation set accuracy: %s: %.2f%%" % (model.metrics_names[1], scores_emb_lrn_rnn[1]*100))

    #Appending validation accuracy to a list 
    vlscores_emb_lrn_rnn.append(scores_emb_lrn_rnn[1] * 100)
...
print("Accuracy score on Validation dataset: %.2f%% (+/- %.2f%%)" % (numpy.mean(vlscores_emb_lrn_rnn), numpy.std(vlscores_emb_lrn_rnn)))

#Testing the last model over our hold-out 20% Test dataset
X_test_emb_lrn_rnn = sequence.pad_sequences(X_test_emb_lrn_rnn, maxlen=max_words)
test_dataset = tf.data.Dataset.from_tensor_slices((X_test_emb_lrn_rnn, Y_test_emb_lrn_rnn))
